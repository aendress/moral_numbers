...
sub print_scenario
{
  ...
  my $label = get_question_id ($scenario->findvalue('./label'),
			      $replacement_hash);
  ...
  push (@out_lines,
	("[[Question:Matrix]]",
	 "[[ID:" . $label . "]]", # Create label from label and number condition

	 '<b>' . $scenario->findvalue('./title') . "</b><br><br>\n",
  
	 replace_vars( $scenario->findvalue('./text'), $replacement_hash ) . "<br><br>\n",

	 "[[AdvancedChoices]]",
	 "[[Choice]]",
	 remove_line_breaks_and_extra_space ("<p style=\"min-width:350px;\">" . replace_vars( $scenario->findvalue('./question.severity.saved'), $replacement_hash ) . "<br></p>"),
	 "[[Choice]]",
	 remove_line_breaks_and_extra_space ("<p style=\"min-width:350px;\">" . replace_vars( $scenario->findvalue('./question.severity.victims'), $replacement_hash ) . "<br></p>"),
	 "[[Choice]]",
	 remove_line_breaks_and_extra_space ("<p style=\"min-width:350px;\">" . replace_vars( $scenario->findvalue('./question.acceptability'), $replacement_hash ) . "<br></p>"),


	 "\n[[AdvancedAnswers]]"));

  
  foreach my $choice (1..$scenario->findvalue('./nchoices')){
